...
try{
  ...
  if(orders.length===0){
    return new ResponseHandler(res, 404, false, "No order found");
  }
  
  return new ResponseHandler(res, 200, true, "Get orders successfully", orders);
}
  catch(error){
     return new ResponseHandler(res, 500, false, error.message)
  }
...
exports.updateStatus = async (req, res) => {
  if (
    ...
    try {
      ...
      new ResponseHandler(res, 200, true, "status Updated");
    } catch (error) {
      return new ResponseHandler(res, 500, false, error.message);
    }
  } else {
